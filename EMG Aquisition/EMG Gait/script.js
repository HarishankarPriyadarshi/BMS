...
function setupJsPlumb() {
  ...
  instance.addEndpoint("box", {
    uuid: "box-elec1",
    anchor: [.605, 0.537, 1, 0],
    isSource: true,
    overlays: [["Label", { label: "E1", location: [1.6, -.15], cssClass: "endpoint-label" }]]
  });
  ...
  instance.addEndpoint("box", {
    ...
    anchor: [0.6, .62, 1, 0],
    isSource: true,
    overlays: [["Label", { label: "E2", location: [1.9, -.1], cssClass: "endpoint-label" }]]
  });

  instance.addEndpoint("box", {
    uuid: "box-elec3",
    anchor: [.636, 0.698, 1, 0],
    isSource: true,
  ...
  document.getElementById("waveform-btn").addEventListener("click", () => {
    ...
    const gaitBtn = document.createElement("button");
    gaitBtn.textContent = "GAIT";
    gaitBtn.className = "round-btn";
    gaitBtn.onclick = () => {
      if (!isPlaying || currentMode !== "gait") {
        isPlaying = true;
        currentMode = "gait";
        prepareGaitData();
        drawGait();
    ...
    waveformOptions.appendChild(gaitBtn);
  });
}
...
// ========== Gait Graph ==========
const emgData = [];
function prepareGaitData() {
  emgData.length = 0;
  for (let i = 0; i < 2000; i++) {
    const burst = Math.sin(i / 50) * (Math.sin(i / 5) > 0.8 ? 1 : 0);
    const noise = (Math.random() - 0.5) * 0.1;
    emgData.push(burst + noise);
  }
}
...
function drawGait() {
  if (!isPlaying || currentMode !== "gait") return;

  ctx.clearRect(0, 0, width, height);

  const yLabels = [-0.5, -0.4, -0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4,];
  ctx.fillStyle = "black";
  ctx.font = "10px Arial";
  ctx.textAlign = "right";
  ...
  for (let i = 0; i <= xTickCount; i++) {
    const labelSec = (scrollPosition + i * visibleSamples / xTickCount) / samplesPerSecond;
  ...
  for (let x = 0; x < visibleSamples; x++) {
    const dataIndex = (scrollPosition + x) % emgData.length;
    const value = emgData[dataIndex];
    const xPos = 40 + (x / visibleSamples) * (width - 60);
    const yPos = centerY - (value / 0.5) * (centerY - 10);
  ...
  scrollPosition = (scrollPosition + samplesPerFrame) % emgData.length;
  animationId = requestAnimationFrame(drawGait);
}
